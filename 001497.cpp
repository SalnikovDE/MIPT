...
AdjListGraph make_knight_moves_graph(size_t N) {
    AdjListGraph graph(N * N, false);
    for (int y = 0 ; y < N; y++) {
        for (int x = 0; x < N; x++) {
            if (y + 2 < N) {

                if (x + 1 < N) {
                    graph.add_edge(encode(x, y, N), encode(x + 1, y + 2, N));
                }
            ...
            if (y + 1 < N) {
                if (x + 2 < N) {
                    graph.add_edge(encode(x, y, N), encode(x + 2, y + 1, N));
                }
...
int main() {
    ...
    AdjListGraph graph = make_knight_moves_graph(N);

    auto path = GraphAlgorithms::find_shortest_way_bfs(graph, encode(x1 - 1, y1 - 1, N), encode(x2 - 1, y2 - 1, N));
    ...
    for (auto const &c : path) {
        std::cout << decode(c, N).first + 1 << ' ' << decode(c, N).second + 1 << std::endl;
    }
}