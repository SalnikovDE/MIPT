...
/*
 ...
 Расстоянием между клетками (x1, y1) и (x2, y2) назовем сумму |x1-x2|+|y1-y2|. Вам необходимо построить таблицу, в клетке (i, j) которой будет записано 
 минимальное расстояние между клеткой (i, j) начальной таблицы и клеткой, в которой записана 1. Гарантируется, что хотя бы одна 1 в таблице есть.
*/
...
AdjListGraph make_graph(size_t x_size, size_t y_size) {
    AdjListGraph graph(x_size * y_size, false);
    for (int y = 0 ; y < y_size; y++) {
        for (int x = 0; x < x_size; x++) {
            if (y + 1 < y_size) {
                graph.add_edge(encode(x, y, x_size), encode(x, y + 1, x_size));
            }
            if (x + 1 < x_size) {
                graph.add_edge(encode(x, y, x_size), encode(x + 1, y, x_size));
            }
        }
    }
    return graph;
...
int main() {
    ...
    table.resize(y_size, std::vector<int>(x_size));
    for (int y = 0; y < y_size; y++) {
        for (int x = 0; x < x_size; x++) {
            std::cin >> table[y][x];
        }
    }
    AdjListGraph graph = make_graph(x_size, y_size);
    std::vector<size_t> dist(x_size * y_size, INT32_MAX);
    for (int y = 0; y < y_size; y++) {
        for (int x = 0; x < x_size; x++) {
            if (table[y][x] == 1) {
                GraphAlgorithms::bfs_find_dist(graph, encode(x, y, x_size), dist);
            }
        }
    }
    ...
    for (int y = 0; y < y_size; y++) {
        for (int x = 0; x < x_size; x++) {
            std::cout << dist[encode(x, y, x_size)] << ' ';
        }
        std::cout << std::endl;
    }
}