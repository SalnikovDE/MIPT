 ...
 * Напишите программу, которая будет находить расстояния в неориентированном взвешенном графе
 с неотрицательными длинами ребер, от указанной вершины до всех остальных. Программа должна работать быстро для больших разреженных графов.
*/
...
public:
    ...
    virtual void union_sets(const T &first, const T &second, int w = 0) {
        ...
        if (get_set_size(first) >= get_set_size(second)) {
            weights[get_set_id(first)] = res_weight;
            weights[get_set_id(second)] = 0;
        } else {
            weights[get_set_id(first)] = 0;
            weights[get_set_id(second)] = res_weight;
        }
    ...
    void add_weight(const T &elem, int w) {
        weights[get_set_id(elem)] += w;
    }

    int get_weight(const T &elem) {
        return weights[get_set_id(elem)];
    }
...
class WeightedAdjListGraph : public AdjListGraph {
public:
    WeightedAdjListGraph(size_t _vertex_count, bool _is_directed) : AdjListGraph(_vertex_count, _is_directed) {

    }

    void add_weighted_edge(const Graph::Vertex &start, const Graph::Vertex &finish, long long int w) {
    ...
    long long int get_weight(const Graph::Vertex &start, const Graph::Vertex &finish) const {
        if (weights.find({start, finish}) == weights.end()) {
            return INT32_MAX;
        }
    ...
    virtual std::map<Edge, long long int> get_all_edges() const {
        return weights;
    }

private:
    std::map<Edge, long long int> weights;
};
...
public:
    WeightedAdjMatrixGraph(size_t _vertex_count, bool _is_directed) : AdjMatrixGraph(_vertex_count, _is_directed) {
        weights.resize(vertex_count, std::vector<long long int>(vertex_count, INT32_MAX));
...
private:
    std::vector<std::vector<long long int>> weights;
};
...
namespace GraphAlgorithms {
    ...
    long long floyd_find_min_path(const WeightedAdjMatrixGraph &graph, const Graph::Vertex &start,
                                  const Graph::Vertex &finish) {
        const long long NOT_SET = -1;
        std::vector<std::vector<int>> distances(graph.get_vertex_count(),
                                                std::vector<int>(graph.get_vertex_count()));

        for (int i = 0; i < graph.get_vertex_count(); ++i) {
            for (int j = 0; j < graph.get_vertex_count(); ++j) {
                distances[i][j] = graph.get_weight(i, j);
            }
        }
            ...
            for (int i = 0; i < graph.get_vertex_count(); ++i) {
                for (int j = 0; j < graph.get_vertex_count(); ++j) {
                    if (distances[i][j] > distances[i][k] + distances[k][j]
                        && distances[i][k] != INT32_MAX && distances[k][j] != INT32_MAX) {
    ...
    long long floyd_print_negative_cicle(const WeightedAdjMatrixGraph &graph) {
        const long long NOT_SET = -1;
        std::vector<std::vector<int>> distances(graph.get_vertex_count(),
                                                std::vector<int>(graph.get_vertex_count()));
    ...
    void floyd_print_path(const std::vector<std::vector<int>> &path_vertex, const Graph::Vertex &start,
                          const Graph::Vertex &finish) {
    }


    std::vector<long long int> ford_find_dist(const WeightedAdjMatrixGraph &graph, const Graph::Vertex &v) {
        const int max_path = 30000;
        std::vector<long long int> dist(graph.get_vertex_count(), max_path);
        dist[v] = 0;
    ...
    long long int ford_find_min_path(const WeightedAdjMatrixGraph &graph, const Graph::Vertex &start,
                                     const Graph::Vertex &finish, int moves) {
        const long long int max_path = INT32_MAX;
        std::vector<std::vector<long long int>> dist(moves + 1,
                                                     std::vector<long long int>(graph.get_vertex_count(), max_path));
        dist[0][start] = 0;

        for (int k = 0; k < moves; ++k) {
            for (int i = 0; i < graph.get_vertex_count(); ++i) {
                for (int u = 0; u < graph.get_vertex_count(); ++u) {
                    if (dist[k][i] != max_path) {
                        dist[k + 1][u] = std::min(dist[k + 1][u], dist[k][i] + graph.get_weight(i, u));
        ...
        return dist[moves][finish];
    }

    long long int min_spanning_tree_weight(const WeightedAdjListGraph &graph) {
        long long int sum = 0;
        std::set<std::pair<long long, Graph::Edge>> sorted_edges;
        DisjointSet<Graph::Vertex> vertex_set(0, graph.get_vertex_count());
        for (auto const &e : graph.get_all_edges()) {
            sorted_edges.insert({e.second, e.first});
        }

        for (auto const &e : sorted_edges) {
            if (!vertex_set.in_same_set(e.second.first, e.second.second)) {
                vertex_set.union_sets(e.second.first, e.second.second);
                sum += e.first;
            }
        }
        return sum;
    ...
    std::vector<unsigned long long> dijkstra_find_distances (const WeightedAdjListGraph &graph, const Graph::Vertex &source) {

        const unsigned long long NO_PATH = 2009000999;
        std::vector<unsigned long long> distances(graph.get_vertex_count(), NO_PATH);
        std::set<std::pair<unsigned long long, Graph::Vertex >> sorted_dist;
        distances[source] = 0;
        for (int i = 0; i < graph.get_vertex_count(); ++i) {
            sorted_dist.insert({distances[i], i});
        }
        while (!sorted_dist.empty()) {
            auto v = *sorted_dist.begin();

            sorted_dist.erase(v);

            for (auto const &i : graph.get_all_neighbours(v.second)) {
                if (distances[i] > v.first + graph.get_weight(v.second, i)) {
                    sorted_dist.erase({distances[i], i});
                    distances[i] = v.first + graph.get_weight(v.second, i);
                    sorted_dist.insert({distances[i], i});
                }
            }
        }
...
int main() {
    ...
    size_t test, vertex_count, edge_count;
    std::cin >> test;
    ...
    for (int t = 0; t < test; ++t) {
        ...
        for (int i = 0; i < edge_count; ++i) {
            ...
            long long w;
            std::cin >> start >> finish >> w;
            graph.add_weighted_edge(start, finish, w);
        }
        Graph::Vertex from;
        std::cin >> from;
        for (const auto &d : GraphAlgorithms::dijkstra_find_distances(graph, from)) {
            std::cout << d << ' ';
        }
        std::cout << '\n';
    }
