...
/*
 * Найти точки сочленения в неор. графе.
*/
...
int main() {
    ...
    auto cutpoints = GraphAlgorithms::find_cutpoints(g);

    std::cout << cutpoints.size() << std::endl;

    for (Graph::Vertex i = 0; i < g.get_vertex_count(); ++i) {
        if (cutpoints.find(i) != cutpoints.end()) {
