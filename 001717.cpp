...
Graph::Vertex move_in_direction (const std::vector<std::vector<char>> &track, int x, int y, int x_dir, int y_dir) {
    int cnt;
    for (cnt = 0; y < track.size() && x < track[y].size() && track[y][x] != '#'; cnt ++) {
        x += x_dir;
        y += y_dir;
    }

    return encode(x - ((cnt + 1) / 2) * x_dir, y - ((cnt + 1) / 2) * y_dir, track[y - ((cnt + 1) / 2) * y_dir].size());
}


std::pair<Graph::Vertex, Graph::Vertex > make_graph (const std::vector<std::vector<char>> &track, Graph &graph) {
    std::pair<Graph::Vertex, Graph::Vertex> res;
    for (int y = 0; y < track.size(); ++y) {
        for (int x = 0; x < track[y].size(); ++x) {
            ...
            if (track[y][x] == 'S') {
                res.first = encode(x, y, track[y].size());
            }

            if (track[y][x] == 'T') {

                res.second = encode(x, y, track[y].size());
            }

            auto start = encode(x, y, track[y].size());
            auto finish = move_in_direction(track, x, y, 1, 0);
            if (finish != start) {
                graph.add_edge(start, finish);
            }

            finish = move_in_direction(track, x, y, -1, 0);
            if (finish != start) {
                graph.add_edge(start, finish);
            }

            finish = move_in_direction(track, x, y, 0, -1);
            if (finish != start) {
                graph.add_edge(start, finish);
            }
...
int main() {
    ...
    track.resize(y_size, std::vector<char>(x_size));
    ...
    AdjListGraph graph(x_size * y_size, true);
    auto v = make_graph(track, graph);

    auto path = GraphAlgorithms::find_shortest_way_bfs(graph, v.first, v.second);
