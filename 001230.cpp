...
/*
 * Витя хочет придумать новую игру с числами. В этой игре от игроков требуется преобразовывать четырехзначные числа 
...
std::string rotate(const std::string a) {
    std::string res;
...
int main() {
    ...
    AdjListGraph g(10000, true);
    for (Graph::Vertex i = 1111; i < 10000; i++) {
        auto str = std::to_string(i);
        if (str.find('0') == std::string::npos) {

            if (std::to_string(i - 1).find('0') == std::string::npos && i - 1 >= 1111) {
                g.add_edge(i, i - 1);
            }
            if (std::to_string(i + 1000).find('0') == std::string::npos && i + 1000 <= 9999) {
                g.add_edge(i, i + 1000);
            }

            g.add_edge(i, std::stoi(rotate(str)));
            g.add_edge(std::stoi(rotate(str)), i);
        }
    }
    auto result = GraphAlgorithms::find_shortest_way_bfs(g, start, finish);
